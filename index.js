...
function teacherScript() {
  ...
  document.querySelectorAll('.tab[data-tab]').forEach(t=>t.onclick=()=>{
    ...
    if(t.dataset.tab==='resize')initResize();
    if(t.dataset.tab==='translate')initTranslate();
    if(t.dataset.tab==='crop')initCrop();
    if(t.dataset.tab==='pdf2img')initPdf2Img();
    if(t.dataset.tab==='ai')initAI();
  });
