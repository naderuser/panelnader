...
export default function QuestionsTab({ cookie }) {
  ...
  const saveEdit = () => {
    ...
    if (editIndex === null) updated.push(editQ);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.ok) Alert.alert("✅", "آزمون با موفقیت ذخیره شد");
      else Alert.alert("خطا", data.error || "خطا در ذخیره");
          ...
          {[
            ...
            <View key={f.key} style={styles.fieldRow}>
              ...
              <TextInput
                ...
                value={meta[f.key] || ""}
                onChangeText={v => setMeta({ ...meta, [f.key]: v })}
                textAlign="right"
