 ...
 *  - برنامه هفتگی با جدول زمان‌بندی و خروجی Word
 *
  ...
  <body><div class="wrap">
    ...
    <div id="dash" class="hidden">
      <div class="tabs">
        ...
        <div class="tab" data-tab="desc">📝 توصیف عملکرد</div>
      ...
      <!-- توصیف عملکرد -->
      <div class="card tab-content hidden" id="tab-desc">
        <div class="section-header">
          <div>
            <h3>📝 توصیف عملکرد پایه‌های اول تا ششم</h3>
            <p class="muted">کارنامه توصیفی دانش‌آموزان دوره ابتدایی</p>
          </div>
        </div>
        <div style="margin-bottom:16px">
          <select id="desc-grade" onchange="renderDesc()" style="padding:10px 16px;border:1px solid var(--line);border-radius:8px;font-size:15px;margin-left:12px">
          ...
          <select id="desc-subject" onchange="renderDesc()" style="padding:10px 16px;border:1px solid var(--line);border-radius:8px;font-size:15px;margin-left:12px"></select>
          <select id="desc-level" onchange="renderDesc()" style="padding:10px 16px;border:1px solid var(--line);border-radius:8px;font-size:15px">
            <option value="excellent">خیلی خوب</option>
            <option value="good">خوب</option>
            <option value="acceptable">قابل قبول</option>
            <option value="needs-effort">نیاز به تلاش بیشتر</option>
          </select>
        </div>
        <div id="desc-result" style="padding:16px;background:var(--card);border:1px solid var(--line);border-radius:12px;min-height:100px;line-height:2"></div>
        <div style="margin-top:12px;display:flex;gap:10px">
          <button class="btn" onclick="descCopy()">📋 کپی</button>
          <button class="btn primary" onclick="descDownload()">📥 دانلود Word</button>
        </div>
      </div>
...
function teacherScript() {
  ...
  const DESC_DATA={
    "1":{
      "قرآن":{"excellent":"با علاقه و اشتیاق فراوان در کلاس قرآن شرکت می‌کند، آداب تلاوت را به‌طور کامل رعایت می‌نماید، آیات و سوره‌های آموزش‌داده‌شده را روان، شمرده، با تجوید صحیح و تلفظ دقیق روخوانی می‌کند.","good":"با رعایت آداب تلاوت، آیات را شمرده و با تلفظ نسبتاً صحیح روخوانی می‌کند. در فعالیت‌های گروهی قرآن فعال است و برخی سوره‌ها را با کمک از حفظ می‌خواند.","acceptable":"با راهنمایی و تشویق معلم، آیات را روخوانی می‌کند، آداب تلاوت را تا حدودی رعایت می‌نماید و در جمع‌خوانی شرکت می‌کند.","needs-effort":"در روخوانی آیات و رعایت تجوید نیاز به تمرین و تلاش بیشتری دارد. مرور روزانه در منزل و تکرار سوره‌ها کمک زیادی به پیشرفت او خواهد کرد."},
      "فارسی":{"excellent":"نشانه‌ها، کلمات و جمله‌ها را کاملاً روان، درست و با لحن مناسب می‌خواند. با دقت به داستان‌ها و متون گوش می‌دهد، سؤالات را به‌خوبی پاسخ می‌دهد و با خلاقیت زیبا سخن می‌گوید.","good":"نشانه‌ها و کلمات را به‌درستی تشخیص می‌دهد، متون را شمرده و با تلفظ مناسب می‌خواند. در فعالیت‌های گفتاری و گوش دادن فعال است.","acceptable":"با کمک و راهنمایی معلم، نشانه‌ها را می‌خواند، در گوش دادن به داستان‌ها توجه نشان می‌دهد و در فعالیت‌های گفتاری شرکت می‌کند.","needs-effort":"در تشخیص سریع نشانه‌ها و خواندن روان کلمات نیاز به تمرین بیشتری دارد. خواندن روزانه کتاب‌های ساده در منزل به پیشرفت او کمک زیادی می‌کند."},
      "ریاضی":{"excellent":"مفاهیم اعداد، الگوها، جمع و تفریق ساده، اندازه‌گیری و شکل‌های هندسی را به‌خوبی درک می‌کند. مسائل را با سرعت و دقت حل می‌نماید.","good":"اعداد را به‌درستی می‌شناسد، عملیات ساده جمع و تفریق را انجام می‌دهد و در شناخت الگوها و شکل‌ها موفق است.","acceptable":"با کمک معلم، مفاهیم پایه ریاضی را یاد می‌گیرد، اعداد را می‌شناسد و در فعالیت‌های عملی شرکت می‌کند.","needs-effort":"در شناخت اعداد، حل مسائل ساده و درک الگوها نیاز به تمرین بیشتری دارد. انجام بازی‌های ریاضی و تمرین در منزل به پیشرفت او کمک می‌کند."},
      "علوم":{"excellent":"با کنجکاوی و علاقه فراوان مشاهده می‌کند، سؤالات هوشمندانه می‌پرسد و درباره بدن خود، حواس پنج‌گانه، گیاهان و جانوران با جزئیات توضیح می‌دهد.","good":"در فعالیت‌های عملی و مشاهده‌ای شرکت فعال دارد، مفاهیم ساده علوم را درک می‌کند و مشاهدات خود را به‌خوبی بیان می‌نماید.","acceptable":"با راهنمایی معلم، مشاهده می‌کند، در آزمایش‌ها شرکت می‌کند و دانسته‌های خود را بیان می‌کند.","needs-effort":"در دقت مشاهده، پرسشگری و بیان دانسته‌های علمی نیاز به تلاش بیشتری دارد. گفتگو درباره طبیعت در منزل به پیشرفت او کمک می‌کند."},
      "هنر":{"excellent":"با خلاقیت و تخیل بالا نقاشی می‌کشد، رنگ‌آمیزی را بسیار زیبا و دقیق انجام می‌دهد و در ساخت کاردستی و فعالیت‌های هنری بسیار موفق و نوآور است.","good":"کارهای دستی و نقاشی را با علاقه انجام می‌دهد، رنگ‌ها را به‌خوبی ترکیب می‌کند و در فعالیت‌های هنری فعال است.","acceptable":"در فعالیت‌های هنری و رنگ‌آمیزی شرکت می‌کند و کارهای خود را با راهنمایی معلم انجام می‌دهد.","needs-effort":"در خلاقیت و دقت در کارهای هنری نیاز به تمرین بیشتری دارد. انجام فعالیت‌های هنری در منزل مفید خواهد بود."},
      "تربیت بدنی":{"excellent":"در حرکات پایه، بازی‌های گروهی و فعالیت‌های ورزشی بسیار هماهنگ، چابک و پر انرژی است. قوانین بازی را به‌خوبی رعایت می‌کند.","good":"حرکات ورزشی را درست و با هماهنگی انجام می‌دهد، در بازی‌های گروهی فعال است و از ورزش لذت می‌برد.","acceptable":"با راهنمایی معلم، حرکات پایه را انجام می‌دهد و در فعالیت‌های ورزشی شرکت می‌کند.","needs-effort":"در هماهنگی بدنی و انجام حرکات نیاز به تمرین بیشتری دارد. فعالیت بدنی منظم در منزل به پیشرفت او کمک می‌کند."},
      "مهارت‌های عمومی":{"excellent":"بسیار منظم، پاکیزه و مسئولیت‌پذیر است، با ادب و احترام کامل با معلم و همکلاسی‌ها رفتار می‌کند و تکالیف را کامل و به‌موقع انجام می‌دهد.","good":"منظم و پاکیزه است، با ادب رفتار می‌کند، در فعالیت‌های گروهی همکاری دارد و تکالیف را با تلاش انجام می‌دهد.","acceptable":"با راهنمایی معلم منظم است، ادب را رعایت می‌کند، در فعالیت‌های گروهی شرکت می‌کند و تکالیف را انجام می‌دهد.","needs-effort":"در نظم، پاکیزگی، همکاری و انجام به‌موقع تکالیف نیاز به تلاش بیشتری دارد. حمایت خانواده و یادآوری روزانه کمک زیادی می‌کند."}
    },
    "2":{
      "قرآن":{"excellent":"با علاقه و اشتیاق زیاد در کلاس قرآن شرکت می‌کند، آداب تلاوت را کاملاً رعایت می‌نماید، آیات و سوره‌ها را روان، با تجوید صحیح روخوانی می‌کند و چندین سوره را از حفظ دارد.","good":"آداب تلاوت را رعایت می‌کند، آیات را شمرده و با تلفظ مناسب روخوانی می‌نماید و برخی سوره‌ها را از حفظ می‌خواند.","acceptable":"با راهنمایی معلم آیات را روخوانی می‌کند، آداب تلاوت را تا حدودی رعایت می‌نماید و در فعالیت‌های گروهی شرکت می‌کند.","needs-effort":"در روخوانی روان آیات، رعایت تجوید و حفظ سوره‌ها نیاز به تمرین بیشتری دارد. مرور روزانه قرآن در منزل کمک زیادی خواهد کرد."},
      "فارسی":{"excellent":"متون درس را کاملاً روان، شمرده و با لحن مناسب می‌خواند، به جزئیات داستان‌ها دقت می‌کند و با خلاقیت زیبا سخن می‌گوید.","good":"در درس هدیه‌های آسمان، مفاهیم اصلی، پیام‌های دینی و داستان‌های قرآنی را به خوبی فرا گرفته است.","acceptable":"با کمک معلم متون را می‌خواند، در گوش دادن به داستان‌ها توجه نشان می‌دهد و در فعالیت‌های گفتاری شرکت می‌کند.","needs-effort":"در خواندن روان و درک دقیق مطالب نیاز به تمرین بیشتری دارد. خواندن روزانه کتاب‌های داستان در منزل بسیار مفید خواهد بود."},
      "ریاضی":{"excellent":"مفاهیم اعداد، جمع و تفریق، ضرب ساده، کسرها و الگوها را به‌خوبی درک می‌کند و مسائل را با سرعت و دقت حل می‌نماید.","good":"عملیات جمع، تفریق و ضرب ساده را به‌درستی انجام می‌دهد و الگوها و شکل‌ها را می‌شناسد.","acceptable":"مفاهیم اصلی مانند جمع و تفریق، شناخت اشکال هندسی و اندازه‌گیری را در سطح قابل قبول فرا گرفته است.","needs-effort":"در درک مفاهیم و حل سریع مسائل نیاز به تمرین بیشتری دارد. انجام بازی‌های ریاضی و تمرین روزانه در منزل کمک خواهد کرد."},
      "علوم":{"excellent":"با کنجکاوی فراوان مشاهده و آزمایش می‌کند و درباره بدن انسان، گیاهان، جانوران و مواد با جزئیات توضیح می‌دهد.","good":"در فعالیت‌های عملی و آزمایش‌ها شرکت فعال دارد و مفاهیم علوم را به‌خوبی درک می‌کند.","acceptable":"با راهنمایی معلم در آزمایش‌ها شرکت می‌کند و دانسته‌های خود را تا حدودی بیان می‌کند.","needs-effort":"در دقت مشاهده و پرسشگری نیاز به تلاش بیشتری دارد. گفتگو درباره طبیعت در منزل مفید است."},
      "هدیه‌های آسمان":{"excellent":"با علاقه و اشتیاق فراوان در کلاس شرکت می‌کند، داستان‌های پیامبران و اهل بیت را با دقت گوش می‌دهد و مفاهیم دینی را در رفتار خود نشان می‌دهد.","good":"مفاهیم اصلی، پیام‌های دینی و داستان‌های قرآنی را به خوبی فرا گرفته است و در فعالیت‌های کلاسی با اشتیاق شرکت می‌کند.","acceptable":"مفاهیم و پیام‌های دینی را در حد انتظار فرا گرفته و با احکام و داستان‌های دینی آشنایی دارد.","needs-effort":"در توجه به درس و درک مفاهیم دینی نیاز به تلاش بیشتری دارد. مرور درس‌ها در منزل کمک زیادی می‌کند."},
      "هنر":{"excellent":"با خلاقیت بالا نقاشی می‌کشد و کاردستی‌های خلاقانه می‌سازد و در فعالیت‌های هنری بسیار نوآور است.","good":"کارهای دستی و نقاشی را با علاقه انجام می‌دهد و رنگ‌ها را به‌خوبی ترکیب می‌کند.","acceptable":"در فعالیت‌های هنری شرکت می‌کند و کارهای خود را با راهنمایی معلم انجام می‌دهد.","needs-effort":"در خلاقیت و دقت کارهای هنری نیاز به تمرین بیشتری دارد. فعالیت هنری در منزل مفید خواهد بود."},
      "تربیت بدنی":{"excellent":"در حرکات ورزشی، بازی‌های گروهی و فعالیت‌های بدنی بسیار هماهنگ، چابک و پر انرژی است و قوانین را رعایت می‌کند.","good":"حرکات ورزشی را درست انجام می‌دهد و در بازی‌های گروهی فعال است.","acceptable":"با راهنمایی معلم حرکات را انجام می‌دهد و در فعالیت‌های ورزشی شرکت می‌کند.","needs-effort":"در هماهنگی بدنی و استقامت نیاز به تمرین بیشتری دارد. فعالیت بدنی منظم در منزل کمک می‌کند."},
      "مهارت‌های عمومی":{"excellent":"بسیار منظم، مسئولیت‌پذیر و همکاری‌کننده است، با ادب و احترام کامل رفتار می‌کند و تکالیف را کامل و به‌موقع تحویل می‌دهد.","good":"منظم است، مسئولیت خود را می‌پذیرد و با دیگران همکاری می‌کند.","acceptable":"با راهنمایی معلم منظم و مسئولیت‌پذیر است و در فعالیت‌های گروهی شرکت می‌کند.","needs-effort":"در نظم، مسئولیت‌پذیری و همکاری نیاز به تلاش بیشتری دارد. برنامه‌ریزی روزانه در منزل کمک می‌کند."}
    },
    "3":{
      "قرآن":{"excellent":"با اشتیاق فراوان در کلاس قرآن شرکت می‌کند، آداب تلاوت را به‌طور کامل رعایت می‌نماید، آیات و سوره‌ها را روان، با تجوید صحیح روخوانی می‌کند و چندین سوره را کاملاً از حفظ دارد.","good":"آداب تلاوت را رعایت می‌کند، آیات را شمرده و با تلفظ مناسب روخوانی می‌نماید و برخی سوره‌ها را از حفظ می‌خواند.","acceptable":"با راهنمایی معلم آیات را روخوانی می‌کند، آداب تلاوت را تا حدودی رعایت می‌نماید و در فعالیت‌های گروهی شرکت می‌کند.","needs-effort":"در روخوانی روان، رعایت تجوید و حفظ سوره‌ها نیاز به تمرین بیشتری دارد. مرور روزانه قرآن در منزل کمک زیادی خواهد کرد."},
      "فارسی":{"excellent":"متون درس را کاملاً روان، با لحن مناسب و رسا می‌خواند، درک مطلب عالی دارد، به جزئیات داستان‌ها و شعرها دقت می‌کند و با خلاقیت زیبا سخن می‌گوید.","good":"متون را به‌درستی و شمرده می‌خواند، درک مطلب خوبی نشان می‌دهد و نظرات خود را به‌خوبی بیان می‌کند.","acceptable":"در درس فارسی، مهارت‌های خواندن و نوشتن را در حد قابل قبول فرا گرفته است. او قادر به درک متون ساده بوده و توانایی نوشتن جملات ابتدایی را دارد.","needs-effort":"در خواندن روان و درک دقیق مطالب نیاز به تمرین بیشتری دارد. خواندن روزانه کتاب‌های داستان در منزل بسیار مفید خواهد بود."},
      "ریاضی":{"excellent":"مفاهیم اعداد، ضرب و تقسیم، کسرها، مساحت، حجم، زمان و مسائل را به‌خوبی درک می‌کند و مسائل را با سرعت و دقت حل می‌نماید.","good":"عملیات ضرب و تقسیم را به‌درستی انجام می‌دهد، کسرها و مفاهیم هندسی را می‌شناسد و در حل مسائل کلاسی موفق است.","acceptable":"در درس ریاضی، مفاهیم پایه مانند جمع و تفریق اعداد سه رقمی و درک اشکال هندسی را در سطح قابل قبول فرا گرفته است. او در حل مسائل ساده توانایی نسبی دارد.","needs-effort":"در درک مفاهیم و حل سریع مسائل نیاز به تمرین بیشتری دارد. بازی‌های ریاضی و تمرین روزانه در منزل به پیشرفت او کمک خواهد کرد."},
      "علوم تجربی":{"excellent":"با کنجکاوی زیاد مشاهده و آزمایش می‌کند، سؤالات هوشمندانه می‌پرسد و درباره بدن انسان، نیرو، مواد، گیاهان، جانوران و محیط زیست با جزئیات توضیح می‌دهد.","good":"در درس علوم تجربی، مفاهیم و مباحث ارائه شده را به خوبی درک کرده و قادر به توضیح ارتباط بین پدیده‌های علمی است. او در انجام آزمایش‌ها با دقت و علاقه شرکت می‌کند.","acceptable":"در درس علوم تجربی، مفاهیم اصلی را در حد قابل قبول فرا گرفته و با برخی از موضوعات علمی آشنایی دارد. او در انجام آزمایش‌ها نیاز به راهنمایی بیشتری دارد.","needs-effort":"در دقت مشاهده و بیان مفاهیم علمیی نیاز به تلاش بیشتری دارد. گفتگو درباره طبیعت در منزل مفید است."},
      "مطالعات اجتماعی":{"excellent":"با علاقه درباره خانواده، مدرسه، محله، شهر، استان و کشور صحبت می‌کند و نقش افراد، مشاغل و قوانین را به‌خوبی می‌شناسد.","good":"مفاهیم مربوط به درس‌ها را به خوبی فرا گرفته و درک مناسبی از ارتباط میان موضوعات اجتماعی و زندگی روزمره نشان می‌دهد. او در فعالیت‌های کلاسی مشارکت فعال دارد.","acceptable":"مفاهیم را در سطح قابل قبول فرا گرفته است. او با موضوعات اصلی آشنایی نسبی دارد و در پاسخ به پرسش‌ها معمولاً نیازمند راهنمایی کوتاه است.","needs-effort":"در درک مفاهیم اجتماعی و بیان دانسته‌ها نیاز به تمرین بیشتری دارد. گفتگو درباره اخبار ساده در منزل کمک می‌کند."},
      "هدیه‌های آسمان":{"excellent":"با اشتیاق فراوان در کلاس شرکت می‌کند، داستان‌های پیامبران و امامان را با دقت گوش می‌دهد و مفاهیم دینی را در رفتار خود نشان می‌دهد.","good":"مفاهیم دینی و آموزه‌های اخلاقی را به خوبی درک کرده و در زندگی خود به کار می‌بندد. او با علاقه در فعالیت‌های کلاسی پیرامون مسائل دینی مشارکت می‌کند.","acceptable":"مفاهیم و آموزه‌های دینی را در حد قابل قبول فرا گرفته و با برخی از موضوعات آشنایی دارد. او در پاسخ به سوالات دینی نیاز به راهنمایی بیشتری دارد.","needs-effort":"در توجه به درس، درک مفاهیم دینی و به‌کار بستن آن‌ها در رفتار نیاز به تلاش بیشتری دارد. مرور درس‌ها در منزل کمک می‌کند."},
      "هنر":{"excellent":"با خلاقیت بالا نقاشی می‌کشد، رنگ‌آمیزی دقیق و زیبا انجام می‌دهد و کاردستی‌های خلاقانه می‌سازد.","good":"کارهای دستی و نقاشی را با علاقه انجام می‌دهد و رنگ‌ها را به‌خوبی ترکیب می‌کند.","acceptable":"در فعالیت‌های هنری شرکت می‌کند و کارهای خود را با راهنمایی معلم انجام می‌دهد.","needs-effort":"در خلاقیت و دقت کارهای هنری نیاز به تمرین بیشتری دارد. فعالیت هنری در منزل مفید خواهد بود."},
      "تربیت بدنی":{"excellent":"در حرکات ورزشی و بازی‌های گروهی بسیار هماهنگ، چابک و پر انرژی است و روحیه همکاری بالایی دارد.","good":"حرکات ورزشی را درست انجام می‌دهد و در بازی‌های گروهی فعال است.","acceptable":"با راهنمایی معلم حرکات را انجام می‌دهد و در فعالیت‌های ورزشی شرکت می‌کند.","needs-effort":"در هماهنگی بدنی و استقامت نیاز به تمرین بیشتری دارد. فعالیت بدنی منظم در منزل کمک می‌کند."},
      "مهارت‌های عمومی":{"excellent":"بسیار منظم، مسئولیت‌پذیر و همکاری‌کننده است، با ادب و احترام کامل رفتار می‌کند و تکالیف را دقیق و خلاقانه انجام می‌دهد.","good":"منظم و مسئولیت‌پذیر است، ادب را رعایت می‌کند و در فعالیت‌ها همکاری دارد.","acceptable":"با راهنمایی معلم منظم است و در کارهای گروهی شرکت می‌کند.","needs-effort":"در نظم، مسئولیت‌پذیری و همکاری نیاز به تلاش بیشتری دارد. برنامه‌ریزی و حمایت خانواده مفید است."}
    },
    "4":{
      "قرآن":{"excellent":"با اشتیاق فراوان در کلاس شرکت می‌کند، آداب تلاوت را کاملاً رعایت می‌نماید، آیات و سوره‌ها را روان با تجوید صحیح روخوانی می‌کند و چندین سوره را از حفظ دارد.","good":"آداب تلاوت را رعایت می‌کند، آیات را شمرده و با تلفظ مناسب روخوانی می‌نماید و برخی سوره‌ها را از حفظ می‌خواند.","acceptable":"با راهنمایی معلم آیات را روخوانی می‌کند، آداب تلاوت را تا حدودی رعایت می‌نماید و در فعالیت‌های گروهی شرکت می‌کند.","needs-effort":"در روخوانی روان آیات، رعایت تجوید و حفظ سوره‌ها نیاز به تمرین بیشتری دارد. مرور روزانه قرآن در منزل کمک زیادی خواهد کرد."},
      "فارسی":{"excellent":"متون درس را کاملاً روان، رسا و با لحن مناسب می‌خواند، درک مطلب عالی دارد و با خلاقیت زیبا سخن می‌گوید.","good":"متون را به‌درستی و شمرده می‌خواند، درک مطلب خوبی نشان می‌دهد و نظرات خود را به‌خوبی بیان می‌کند.","acceptable":"با کمک معلم متون را می‌خواند، در گوش دادن به درس‌ها توجه نشان می‌دهد و در فعالیت‌های گفتاری شرکت می‌کند.","needs-effort":"در خواندن روان و درک دقیق مطالب نیاز به تمرین بیشتری دارد. خواندن روزانه کتاب‌های داستان در منزل بسیار مفید خواهد بود."},
      "ریاضی":{"excellent":"مفاهیم اعداد بزرگ، ضرب و تقسیم چندرقمی، کسرها، مساحت و محیط شکل‌ها را به‌طور عمیق درک می‌کند و مسائل را با روش‌های خلاقانه حل می‌نماید.","good":"عملیات ضرب، تقسیم و کار با کسرها را به‌درستی انجام می‌دهد و مسائل استاندارد کلاسی را با موفقیت حل می‌کند.","acceptable":"مفاهیم پایه‌ای درس ریاضی مانند جمع، تفریق و ضرب اعداد صحیح و شناخت اولیه کسرهای متعارفی را در سطح قابل قبول کسب کرده است.","needs-effort":"در درک عمیق مفاهیم و حل سریع مسائل نیاز به تمرین بیشتری دارد. انجام بازی‌های ریاضی و حل مسائل روزانه در منزل کمک شایانی خواهد کرد."},
      "علوم تجربی":{"excellent":"با کنجکاوی و اشتیاق زیاد مشاهده و آزمایش می‌کند و درباره مغناطیس، نور و صدا، نیرو و حرکت، مواد و بدن انسان با جزئیات توضیح می‌دهد.","good":"مفاهیم علمی را به خوبی درک کرده و توانایی بالایی در مشاهده، آزمایش و تحلیل نتایج نشان می‌دهد. او در ارائه گزارش‌های علمی و استنتاج‌های منطقی دقیق عمل می‌کند.","acceptable":"او مفاهیم اصلی مانند شناخت اجزای محیط اطراف و ویژگی‌های ابتدایی موجودات زنده را آموخته و در فعالیت‌های کلاسی مشارکت نسبی دارد.","needs-effort":"در دقت مشاهده و بیان علمی مفاهیم نیاز به تلاش بیشتری دارد. انجام آزمایش‌های ساده در منزل و گفتگو درباره پدیده‌های اطراف به پیشرفت او کمک می‌کند."},
      "مطالعات اجتماعی":{"excellent":"با علاقه فراوان درباره خانواده، محله، شهر و استان، ایران عزیز و فعالیت‌های اقتصادی صحبت می‌کند و نقش افراد و قوانین را به‌خوبی می‌شناسد.","good":"مفاهیم مربوط به خانواده، اجتماع و محیط پیرامون خود را به خوبی درک کرده و توانایی بالایی در تحلیل روابط اجتماعی و مسئولیت‌های شهروندی نشان می‌دهد.","acceptable":"مفاهیم پایه‌ای مانند آشنایی با خانواده، محله و برخی نهادهای اجتماعی را در سطح قابل قبول فرا گرفته است. او در پاسخ به سوالات و تحلیل مسائل اجتماعی نیاز به راهنمایی بیشتری دارد.","needs-effort":"در درک عمیق مفاهیم شهروندی و جغرافیایی نیاز به تمرین بیشتری دارد. نگاه کردن به نقشه ایران و گفتگو در منزل کمک زیادی می‌کند."},
      "هدیه‌های آسمان":{"excellent":"با اشتیاق و توجه کامل در کلاس شرکت می‌کند و مفاهیم مهم دینی مانند توحید، نبوت، امامت و اخلاق اسلامی را در رفتار خود نشان می‌دهد.","good":"دانش‌آموز مفاهیم کلیدی درس هدیه‌های آسمانی را به خوبی فرا گرفته است. او در فعالیت‌های کلاسی با موضوعات دینی، حضوری فعال و مؤثر دارد.","acceptable":"مفاهیم و آموزه‌های دینی را در حد قابل قبول فرا گرفته و با برخی از موضوعات درس آشنایی دارد. او در پاسخ به سوالات و بیان احکام، نیاز به راهنمایی بیشتری دارد.","needs-effort":"در توجه کامل به درس و درک عمیق مفاهیم دینی نیاز به تلاش بیشتری دارد. مرور درس‌ها و خواندن داستان‌های دینی در منزل کمک می‌کند."},
      "هنر":{"excellent":"با خلاقیت و تخیل بسیار بالا نقاشی می‌کشد و کاردستی‌های خلاقانه و نوآورانه می‌سازد.","good":"کارهای دستی و نقاشی را با علاقه و دقت مناسب انجام می‌دهد و رنگ‌ها را به‌خوبی ترکیب می‌کند.","acceptable":"در فعالیت‌های هنری شرکت می‌کند و کارهای خود را با راهنمایی معلم به پایان می‌رساند.","needs-effort":"در خلاقیت و دقت در جزئیات کارهای هنری نیاز به تمرین بیشتری دارد."},
      "تربیت بدنی":{"excellent":"در حرکات ورزشی و بازی‌های گروهی بسیار هماهنگ، چابک و پر انرژی است و قوانین بازی را کاملاً رعایت می‌کند.","good":"حرکات ورزشی را درست و با هماهنگی انجام می‌دهد و در بازی‌های گروهی فعال است.","acceptable":"با راهنمایی معلم حرکات پایه را انجام می‌دهد و در فعالیت‌های ورزشی شرکت می‌کند.","needs-effort":"در هماهنگی بدنی و استقامت نیاز به تمرین بیشتری دارد."},
      "مهارت‌های عمومی":{"excellent":"بسیار منظم، پاکیزه و مسئولیت‌پذیر است و در کارهای گروهی پیشرو و همکاری‌کننده است.","good":"منظم و مسئولیت‌پذیر است و ادب و احترام را رعایت می‌کند.","acceptable":"با یادآوری معلم منظم است و در کارهای گروهی شرکت می‌کند.","needs-effort":"در نظم، مسئولیت‌پذیری و همکاری نیاز به تلاش بیشتری دارد."}
    },
    "5":{
      "قرآن":{"excellent":"با اشتیاق و علاقه زیاد در کلاس قرآن شرکت می‌کند، آداب تلاوت را کامل رعایت می‌کند، آیات را روان با تجوید صحیح روخوانی می‌کند و چندین سوره را از حفظ دارد.","good":"آداب تلاوت را به‌خوبی رعایت می‌کند، آیات را شمرده و با تلفظ مناسب روخوانی می‌نماید و بیشتر سوره‌ها را با کمی مرور از حفظ می‌خواند.","acceptable":"با راهنمایی معلم آداب را رعایت می‌کند، آیات را روخوانی می‌کند و در جمع‌خوانی شرکت می‌نماید.","needs-effort":"در رعایت آداب، روخوانی روان و حفظ سوره‌ها نیاز به تمرین بیشتری دارد. مرور روزانه قرآن در منزل کمک زیادی خواهد کرد."},
      "فارسی":{"excellent":"متون درس را کاملاً روان و رسا می‌خواند، درک مطلب عمیق دارد، به جزئیات و پیام متن دقت می‌کند و با واژگان غنی زیبا سخن می‌گوید.","good":"دانش‌آموز با علاقه در فعالیت‌های فارسی شرکت می‌کند، متن‌ها را روان می‌خواند و مفهوم کلی آن‌ها را درک می‌کند. در نگارش و املا عملکرد مناسبی دارد.","acceptable":"دانش‌آموز در خواندن، درک مطلب و نگارش به سطح قابل قبولی دست یافته است، اما برای افزایش دقت در روخوانی، املا و بیان منظور خود در نوشتار به تمرین و تلاش بیشتری نیاز دارد.","needs-effort":"در خواندن روان و درک عمیق جزئیات نیاز به تمرین بیشتری دارد. خواندن روزانه کتاب‌های داستان در منزل بسیار مفید خواهد بود."},
      "ریاضی":{"excellent":"مفاهیم را عمیق درک می‌کند، مسائل را با سرعت و دقت حل می‌نماید و در فعالیت‌های عملی و گروهی بسیار فعال و نوآور است.","good":"دانش‌آموز مفاهیم و مهارت‌های ریاضی را به خوبی فراگرفته و در حل تمرین‌ها و مسئله‌ها عملکرد مناسبی دارد. با دقت و علاقه در فعالیت‌های کلاسی شرکت می‌کند.","acceptable":"مفاهیم اصلی ریاضی را در حد قابل قبول فراگرفته است، اما برای افزایش سرعت و دقت در محاسبات و حل مسئله به تمرین بیشتری نیاز دارد.","needs-effort":"در درک عمیق مفاهیم و حل سریع مسائل نیاز به تمرین بیشتری دارد. حل مسائل روزانه و بازی‌های ریاضی در منزل به پیشرفت او کمک خواهد کرد."},
      "علوم تجربی":{"excellent":"با کنجکاوی زیاد مشاهده و آزمایش می‌کند، سؤالات هوشمندانه می‌پرسد و مفاهیم را با جزئیات و مثال‌های واقعی توضیح می‌دهد.","good":"مفاهیم علوم را به خوبی درک کرده و در فعالیت‌های علمی و آزمایشگاهی با علاقه و دقت مشارکت می‌کند. توانایی مناسبی در مشاهده، پرسشگری و نتیجه‌گیری دارد.","acceptable":"مفاهیم اصلی علوم را در حد قابل قبول فراگرفته است، اما برای افزایش دقت در مشاهده، انجام فعالیت‌های علمی و درک بهتر برخی مفاهیم به تمرین و تلاش بیشتری نیاز دارد.","needs-effort":"در دقت مشاهده، پرسشگری و بیان علمی نیاز به تلاش بیشتری دارد. انجام آزمایش‌های ساده در منزل و گفتگو درباره طبیعت مفید است."},
      "مطالعات اجتماعی":{"excellent":"با علاقه درباره تاریخ، جغرافیا، اقتصاد و حقوق شهروندی صحبت می‌کند، نقش افراد و قوانین را به‌خوبی می‌شناسد و در فعالیت‌ها مسئولیت‌پذیر است.","good":"مطالب درسی را به خوبی فراگرفته و در بحث‌ها و فعالیت‌های کلاسی مشارکت فعالی دارد. با علاقه به یادگیری موضوعات مختلف می‌پردازد.","acceptable":"مطالب درسی را در حد قابل قبول آموخته است، اما برای افزایش دقت در یادگیری و مشارکت بیشتر در فعالیت‌های کلاسی به تمرین و تلاش بیشتری نیاز دارد.","needs-effort":"در درک عمیق مفاهیم و بیان دانسته‌ها نیاز به تمرین بیشتری دارد. گفتگو درباره نقشه و اخبار ساده در منزل کمک می‌کند."},
      "هدیه‌های آسمان":{"excellent":"با اشتیاق کامل در کلاس شرکت می‌کند، داستان‌های دینی را با دقت بازگو می‌کند و مفاهیم توحید، نبوت و اخلاق را در رفتار روزمره نشان می‌دهد.","good":"دانش‌آموز با دقت و تمرکز بالا، آموزه‌های مربوط به سیره پیامبران و ائمه (ع) را در این نوبت فرا گرفته است. او توانایی خوبی در تشخیص رفتارهای اخلاقی صحیح از نادرست دارد.","acceptable":"موضوعات کلی درس، توانسته است در فعالیت‌های پایه شرکت کند، اما برای درک عمیق‌تر داستان‌ها و جزئیات مربوط به سیره معصومین، نیاز به مطالعه بیشتر و تمرین دارد.","needs-effort":"در توجه به درس، درک مفاهیم و به‌کار بستن آن‌ها در رفتار نیاز به تلاش بیشتری دارد. مرور درس‌ها در منزل مفید است."},
      "هنر":{"excellent":"با خلاقیت بالا نقاشی می‌کشد، رنگ‌آمیزی زیبا انجام می‌دهد، کاردستی‌های نوآورانه می‌سازد و در فعالیت‌های هنری بسیار موفق است.","good":"کارهای دستی و نقاشی را با علاقه و دقت انجام می‌دهد و رنگ‌ها را به‌خوبی ترکیب می‌کند.","acceptable":"در فعالیت‌های هنری شرکت می‌کند و کارهای خود را با راهنمایی معلم انجام می‌دهد.","needs-effort":"در خلاقیت و دقت کارهای هنری نیاز به تمرین بیشتری دارد. فعالیت هنری در منزل مفید خواهد بود."},
      "تربیت بدنی":{"excellent":"در حرکات و بازی‌های گروهی بسیار هماهنگ، چابک و پر انرژی است، قوانین را کامل رعایت می‌کند و روحیه همکاری و رهبری بالایی دارد.","good":"حرکات ورزشی را درست انجام می‌دهد، در بازی‌ها فعال است و از ورزش لذت می‌برد.","acceptable":"با راهنمایی معلم حرکات را انجام می‌دهد و در فعالیت‌های ورزشی شرکت می‌کند.","needs-effort":"در هماهنگی بدنی، استقامت و اجرای صحیح حرکات نیاز به تمرین بیشتری دارد. فعالیت بدنی منظم کمک می‌کند."},
      "مهارت‌های عمومی":{"excellent":"بسیار منظم، مسئولیت‌پذیر و همکاری‌کننده است، با ادب و احترام کامل رفتار می‌کند و در فعالیت‌های گروهی و کلاسی پیشرو است.","good":"منظم است، مسئولیت خود را می‌پذیرد، با دیگران همکاری می‌کند و تکالیف را با تلاش انجام می‌دهد.","acceptable":"با راهنمایی معلم منظم و مسئولیت‌پذیر است و در فعالیت‌های گروهی شرکت می‌کند.","needs-effort":"در نظم، مسئولیت‌پذیری، همکاری و دقت در انجام تکالیف نیاز به تلاش بیشتری دارد. برنامه‌ریزی روزانه و تشویق در منزل کمک زیادی می‌کند."}
    },
    "6":{
      "قرآن":{"excellent":"با اشتیاق و علاقه زیاد در کلاس قرآن شرکت می‌کند، آداب تلاوت را کامل رعایت می‌کند، آیات را روان با تجوید صحیح روخوانی می‌کند و چندین سوره را از حفظ دارد.","good":"آداب تلاوت را به‌خوبی رعایت می‌کند، آیات را شمرده و با تلفظ مناسب روخوانی می‌نماید و بیشتر سوره‌ها را با کمی مرور از حفظ می‌خواند.","acceptable":"با راهنمایی معلم آداب را رعایت می‌کند، آیات را روخوانی می‌کند و در جمع‌خوانی شرکت می‌نماید.","needs-effort":"در رعایت آداب، روخوانی روان و حفظ سوره‌ها نیاز به تمرین بیشتری دارد. مرور روزانه قرآن در منزل کمک زیادی خواهد کرد."},
      "فارسی":{"excellent":"متون درس را کاملاً روان و رسا می‌خواند، درک مطلب عمیق دارد، به جزئیات، آرایه‌ها و پیام متن دقت می‌کند و با واژگان غنی زیبا سخن می‌گوید.","good":"دانش‌آموز با توانایی بسیار بالا در تحلیل متون ادبی، توانسته است لایه‌های معنایی و نکات ظریف نویسنده را به درستی استخراج کند. او در تشخیص آرایه‌های ادبی مانند تشبیه و استعاره، دقت و هوشمندی فوق‌العاده‌ای از خود نشان می‌دهد.","acceptable":"مهارت‌های پایه را به خوبی کسب کرده است. او در خواندن متن و پاسخ به سوالات مستقیم موفق است، اما برای رسیدن به درک عمیق‌تر و تحلیل نکات ادبی و پیام‌های نهفته در متن، نیاز به تمرین و مطالعه بیشتر دارد.","needs-effort":"در خواندن روان و درک عمیق جزئیات نیاز به تمرین بیشتری دارد. خواندن روزانه کتاب‌های داستان در منزل بسیار مفید خواهد بود."},
      "ریاضی":{"excellent":"مفاهیم نسبت، تناسب، درصد، هندسه و آمار را عمیق درک می‌کند، مسائل پیچیده را با سرعت و روش‌های متنوع حل می‌نماید و در فعالیت‌های گروهی نوآور است.","good":"عملیات ریاضی و مفاهیم را به‌درستی انجام می‌دهد، مسائل کلاسی را با دقت حل می‌کند و در فعالیت‌ها شرکت فعال دارد.","acceptable":"با کمک معلم مفاهیم را یاد می‌گیرد، عملیات پایه را انجام می‌دهد و در فعالیت‌های کلاسی شرکت می‌کند.","needs-effort":"در درک عمیق مفاهیم و حل مسائل چندمرحله‌ای نیاز به تمرین بیشتری دارد. حل مسائل روزانه و بازی‌های ریاضی در منزل به تقویت پایه او کمک می‌کند."},
      "علوم تجربی":{"excellent":"با کنجکاوی و اشتیاق فراوان مشاهده و آزمایش می‌کند، سؤالات هوشمندانه می‌پرسد و مفاهیم علمی را با استدلال قوی و جزئیات دقیق توضیح می‌دهد.","good":"مراحل انجام آزمایش‌های کلاسی را دنبال کرده و مشاهدات خود را به درستی ثبت می‌کند. او توانایی بسیار خوبی در تشخیص متغیرها و درک روابط بین پدیده‌ها دارد.","acceptable":"حقایق علمی و اطلاعات پایه را به خوبی یاد می‌گیرد، اما برای رسیدن به سطح بالاتر، نیاز به تمرین بیشتر در زمینه تحلیل پدیده‌ها و توضیح \"چراها\" دارد.","needs-effort":"در دقت مشاهده، پرسشگری و بیان مفاهیم علمی نیاز به تلاش بیشتری دارد. انجام آزمایش‌های ساده در منزل و گفتگو درباره پدیده‌های اطراف کمک می‌کند."},
      "مطالعات اجتماعی":{"excellent":"با علاقه درباره تاریخ معاصر، جغرافیا، اقتصاد، حقوق شهروندی و مسائل جهانی صحبت می‌کند، نقش افراد و قوانین را به‌خوبی می‌شناسد و در فعالیت‌ها مسئولیت‌پذیر است.","good":"توانسته است ارتباط میان وقایع تاریخی و تأثیر آن‌ها بر جامعه امروزی را به خوبی تحلیل کند. او با دقت نظر بالا، حقوق و مسئولیت‌های شهروندی را شناخته.","acceptable":"او توانایی یادگیری اطلاعات پایه تاریخی و جغرافیایی را دارد، اما برای درک عمیق‌تر روابط علت و معلولی در رویدادهای اجتماعی و تاریخی، نیاز به تمرین و توجه بیشتری دارد.","needs-effort":"در درک عمیق مفاهیم و بیان دانسته‌ها نیاز به تمرین بیشتری دارد. گفتگو درباره نقشه و اخبار در منزل کمک می‌کند."},
      "هدیه‌های آسمان":{"excellent":"با اشتیاق کامل در کلاس شرکت می‌کند، داستان‌های دینی را با دقت بازگو می‌کند و مفاهیم توحید، نبوت، معاد و اخلاق را در رفتار روزمره نشان می‌دهد.","good":"با پایبندی به اصول اخلاقی آموخته شده از درس، رفتاری سنجیده و مؤدبانه از خود نشان می‌دهد. او در رعایت احترام به دیگران، صداقت در گفتار و رفتار، و کمک به هم‌نوعان، گام‌های مؤثری برداشته است.","acceptable":"او با مفاهیم اصلی درس مانند خداشناسی و پیامبران آشنایی دارد، اما برای رسیدن به سطوح بالاتر، نیاز به تمرین و پایبندی بیشتر در انجام فرایض دینی مانند نماز و رعایت احترام به والدین دارد.","needs-effort":"در توجه به درس، درک مفاهیم و به‌کار بستن آن‌ها در رفتار نیاز به تلاش بیشتری دارد. مرور درس‌ها در منزل مفید است."},
      "تفکر و پژوهش":{"excellent":"با خلاقیت و دقت بالا مسئله‌شناسی می‌کند، سؤالات پژوهشی طرح می‌کند، اطلاعات جمع‌آوری می‌کند و گزارش‌های منظم و نوآورانه ارائه می‌دهد.","good":"او در مواجهه با چالش‌ها، ابتدا مسئله را به دقت تعریف کرده، سپس با طرح سوالات هوشمندانه، به دنبال یافتن اطلاعات مرتبط می‌گردد. توانایی او در ارزیابی منابع و نتیجه‌گیری منطقی بر اساس شواهد، بسیار قابل تقدیر است.","acceptable":"او در جمع‌آوری اطلاعات مشارکت می‌کند، اما برای ارزیابی دقیق منابع و تشخیص اطلاعات معتبر از غیرمعتبر، نیاز به راهنمایی و تمرین بیشتری دارد.","needs-effort":"در مسئله‌شناسی، جمع‌آوری اطلاعات و گزارش‌نویسی نیاز به تمرین بیشتری دارد."},
      "هنر":{"excellent":"با خلاقیت بالا نقاشی می‌کشد، رنگ‌آمیزی زیبا انجام می‌دهد، کاردستی‌های نوآورانه می‌سازد و در فعالیت‌های هنری بسیار موفق است.","good":"کارهای دستی و نقاشی را با علاقه و دقت انجام می‌دهد و رنگ‌ها را به‌خوبی ترکیب می‌کند.","acceptable":"در فعالیت‌های هنری شرکت می‌کند و کارهای خود را با راهنمایی معلم انجام می‌دهد.","needs-effort":"در خلاقیت و دقت کارهای هنری نیاز به تمرین بیشتری دارد."},
      "تربیت بدنی":{"excellent":"در حرکات و بازی‌های گروهی بسیار هماهنگ، چابک و پر انرژی است، قوانین را کامل رعایت می‌کند و روحیه همکاری و رهبری بالایی دارد.","good":"حرکات ورزشی را درست انجام می‌دهد، در بازی‌ها فعال است و از ورزش لذت می‌برد.","acceptable":"با راهنمایی معلم حرکات را انجام می‌دهد و در فعالیت‌های ورزشی شرکت می‌کند.","needs-effort":"در هماهنگیی بدنی، استقامت و اجرای صحیح حرکات نیاز به تمرین بیشتری دارد."},
      "مهارت‌های عمومی":{"excellent":"بسیار منظم، مسئولیت‌پذیر و همکاری‌کننده است، با ادب و احترام کامل رفتار می‌کند و در فعالیت‌های گروهی و کلاسی پیشرو است.","good":"منظم است، مسئولیت خود را می‌پذیرد، با دیگران همکاری می‌کند و تکالیف را با تلاش انجام می‌دهد.","acceptable":"با راهنمایی معلم منظم و مسئولیت‌پذیر است و در فعالیت‌های گروهی شرکت می‌کند.","needs-effort":"در نظم، مسئولیت‌پذیری، همکاری و دقت در انجام تکالیف نیاز به تلاش بیشتری دارد."}
    }
  };
  const LEVEL_LABELS={"excellent":"خیلی خوب","good":"خوب","acceptable":"قابل قبول","needs-effort":"نیاز به تلاش بیشتر"};
  
  function renderDesc(){
    var grade=document.getElementById('desc-grade').value;
    var subject=document.getElementById('desc-subject').value;
    var level=document.getElementById('desc-level').value;
    var subs=document.getElementById('desc-subject');
    if(!subs.options.length){
      var subjects=Object.keys(DESC_DATA[grade]||{});
      subs.innerHTML=subjects.map(function(s){return'<option value="'+s+'">'+s+'</option>';}).join('');
      subject=subs.value;
    }
    var text=(DESC_DATA[grade]&&DESC_DATA[grade][subject]&&DESC_DATA[grade][subject][level])||'متنی یافت نشد';
    document.getElementById('desc-result').innerHTML='<b>'+subject+' - '+LEVEL_LABELS[level]+':</b><br><br>'+text;
    window._descCurrent={grade:grade,subject:subject,level:level,text:text};
  }
  
  document.getElementById('desc-grade').onchange=function(){
    document.getElementById('desc-subject').innerHTML='';
    renderDesc();
  };
  
  window.descCopy=function(){
    if(!window._descCurrent){toast('ابتدا یک توصیف انتخاب کنید');return;}
    navigator.clipboard.writeText(window._descCurrent.subject+' - '+LEVEL_LABELS[window._descCurrent.level]+': '+window._descCurrent.text).then(function(){toast('کپی شد ✅');});
  };
  
  window.descDownload=function(){
    if(!window._descCurrent){toast('ابتدا یک توصیف انتخاب کنید');return;}
    var d=window._descCurrent;
    var html='<html dir="rtl"><head><meta charset="utf-8"><title>توصیف عملکرد</title></head><body style="font-family:B Nazanin,Arial,sans-serif;font-size:14pt;line-height:2;padding:30px">';
    html+='<h2 style="text-align:center">توصیف عملکرد پایه '+d.grade+'</h2>';
    html+='<h3>'+d.subject+' - '+LEVEL_LABELS[d.level]+'</h3>';
    html+='<p>'+d.text+'</p>';
    html+='</body></html>';
    var blob=new Blob([html],{type:'application/msword'});
    var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='توصیف-عملکرد.doc';a.click();
    toast('دانلود شد ✅');
  };
  renderDesc();
  // ---- اسکنر عکس حرفه‌ای ----
